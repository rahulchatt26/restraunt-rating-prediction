...
from restaurant.config import TARGET_COLUMN,PREDICTOR_FLOAT_COLUMN,PREDICTOR_INT_CLOUMNS_LIST
from restaurant.utils import convert_columns_float,write_yaml_file,convert_columns_int,encode_categorical_columns
from restaurant.config import PREDICTOR_CATEGORICAL_COLUMNS_LIST
...
class DataValidation:
    ...
    def initiate_data_validation(self)->DataValidationArtifact:
        try:
            ...
            # base_df = convert_columns_float(df=base_df, include_columns=include_columns)
            # train_df = convert_columns_float(df=train_df, include_columns=include_columns)
            # test_df = convert_columns_float(df=test_df, include_columns=include_columns)
            ...
            # base_df = convert_columns_int(df=base_df, include_columns=include_columns)
            # train_df = convert_columns_int(df=train_df, include_columns=include_columns)
            # test_df = convert_columns_int(df=test_df, include_columns=include_columns)
            ...
            # base_df = encode_categorical_columns(df=base_df, include_columns=include_columns)
            # train_df = encode_categorical_columns(df=train_df, include_columns=include_columns)
            # test_df = encode_categorical_columns(df=test_df, include_columns=include_columns)
